...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        if file_type == "train":
            if args.train_data_file.endswith('.json'):
                    self.convert_json_to_csv(args.train_data_file, 'train_data.csv')
                    file_path='train_data.csv'
            else:
                    file_path = args.train_data_file
        elif file_type == "eval":
            if args.eval_data_file.endswith('.json'):
                    self.convert_json_to_csv(args.eval_data_file, 'eval_data.csv')
                    file_path='eval_data.csv'
            else:
                    file_path = args.eval_data_file
        elif file_type == "test":
            if args.test_data_file.endswith('.json'):
                    self.convert_json_to_csv(args.test_data_file, 'test_data.csv')
    ...
    def convert_json_to_csv(self, json_file_path, csv_file_path):
...
def convert_json_to_csv_a(json_file_path, csv_file_path):
        with open(json_file_path, 'r') as json_file:
             data= json.load(json_file)
...
def generate_result_df(logits, y_trues, y_preds, args):
    print(len(logits), len(y_trues), len(y_preds))
    if args.test_data_file.endswith('.json'):
        convert_json_to_csv_a(args.test_data_file, 'test_data.csv')
        file_path='test_data.csv'
    else:
        file_path = args.test_data_file
    ...
    all_num_flaw_lines=[0]*len(logits)
    print(len(logits), len(y_trues), len(y_preds), len(all_num_flaw_lines), len(all_num_flaw_lines), len(all_num_lines))
    assert len(logits) == len(y_trues) == len(y_preds) == len(all_num_flaw_lines)
...
def write_raw_preds_csv(args, y_preds):
    ...
    df.to_csv("./results/raw_preds_reveal.csv", index=False)
